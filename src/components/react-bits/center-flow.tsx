...
const CenterFlow: React.FC<CenterFlowProps> = ({
  ...
  centerBackground = "rgba(10, 10, 10, 0.85)",
  centerBorderColor,
  centerFillsArea = false,
  className,
}) => {
  ...
  useEffect(() => {
    ...
    const updateGlow = () => {
      if (glowRef.current) {
        ...
        glowRef.current.style.borderColor =
          centerBorderColor ?? `${glowColor}${alpha}`;
  ...
  }, [glowColor, glowDecay, centerBorderColor]);
  ...
  const nodeStyle = useMemo(
    () => ({
      ...
      boxShadow: `0 4px 18px rgba(0,0,0,0.45), 0 0 14px ${glowColor}10`,
    }),
  ...
  const innerCenterSize = centerFillsArea ? centerSize : centerSize * 0.5;
    ...
    >
      ...
      >
        <div
          ...
          style={{ width: innerCenterSize, height: innerCenterSize }}
        >
